...
def to_include(letter, args_list):
	args_parsed = [x for x in ' '.join(args_list).split('-') if len(x) > 0]
...
if __name__ == '__main__':
	...
	if len(args) > 0:
		def include(letter):
			return to_include(letter, args)

		conjugations_to_include = include('c')
		moods_to_include = include('m')
		tenses_to_include = include('t')
		voices_to_include = include('v')
